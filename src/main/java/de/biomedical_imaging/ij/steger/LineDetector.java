...
public class LineDetector {
	...
	public Lines detectLines(ImageProcessor ip, double sigma,
			...
			boolean doExtendLine, double slopeThreshold) {
		return detectLines(ip, sigma, upperThresh, lowerThresh, minLength, maxLength,isDarkLine,
			doCorrectPosition, doEstimateWidth, doExtendLine, OverlapOption.NONE, slopeThreshold);
	}

	public Lines detectLines(ImageProcessor ip, double sigma,
		double upperThresh, double lowerThresh, double minLength, double maxLength, boolean isDarkLine,
		boolean doCorrectPosition, boolean doEstimateWidth,
		boolean doExtendLine, OverlapOption overlapOption, double slopeThreshold) {
	...
	lines = get_lines(sigma, upperThresh, lowerThresh, minLength, maxLength, ip.getHeight(),
			ip.getWidth(), ip, junctions, overlapOption, slopeThreshold);

	// Filter out non-horizontal lines
        Lines horizontalLines = new Lines(lines.getFrame());
        for (Line line : lines) {
            double slope = line.getSlope();
            if (Math.abs(slope) < slopeThreshold) { // Adjust this value as needed
...
/*	private void deleteContour(Lines contours, Junctions junctions, Line c) {
    ...
    private Lines get_lines(double sigma, double high, double low, double minLength, double maxLength, int rows,
			int cols, ImageProcessor in_img, Junctions resultJunction, OverlapOption overlapOption, double slopeThreshold) {
		...
		opts = new Options(-1.0, -1.0, -1.0, isDarkLine ? LinesUtil.MODE_DARK
				: LinesUtil.MODE_LIGHT, -1.0, -1.0, slopeThreshold,doCorrectPosition, doEstimateWidth,
				doExtendLine, false, false, false, overlapOption);

		opts.sigma = sigma;
		opts.high = high;
		opts.low = low;
		opts.slopeThreshold = slopeThreshold;
